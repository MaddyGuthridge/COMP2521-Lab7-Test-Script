...
// Original implementation of showState
static void showStateOld(Maze m, Cell c) {
    int h = MazeHeight(m);
    int w = MazeWidth(m);
...
static bool isNullCell(int r, int c)
{
    return (r == -1 && c == -1);
}
...
static void printBlock(Maze m, int row, int col, Cell c)
{
    if (m->state[row][col] == PATH) {
        printf("\033[43m  \033[0m");
    } else if (row == c.row && col == c.col) {
...
static void showState(Maze m, Cell c) {
    ...
    if (!isNullCell(prev_row, prev_col))
    {
        // Update previous cell
        Cell pos = getInputCoords(prev_row, prev_col);
        GOTO_POS(pos.row, pos.col);
        printBlock(m, prev_row, prev_col, c);
    }

    // print new cell
    Cell pos = getInputCoords(c.row, c.col);
    GOTO_POS(pos.row, pos.col);
    printBlock(m, c.row, c.col, c);

    // set cursor back to end of maze so we don't have any printing errors if this is the end
    GOTO_POS(h + 1, 0);
